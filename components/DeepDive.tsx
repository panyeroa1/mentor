...
export const DeepDive: React.FC = () => {
    ...
    return (
        <div className="h-full flex flex-col">
            <h2 className="text-2xl font-bold text-red-400 mb-2">Deep Dive Analysis</h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <textarea
                    ...
                    className="w-full bg-gray-800 text-white rounded-lg p-3 h-48 focus:outline-none focus:ring-2 focus:ring-red-500"
                    disabled={isLoading}
                />
                <button
                    type="submit"
                    disabled={isLoading || !prompt.trim()}
                    className="bg-red-600 text-white font-bold py-3 px-5 rounded-lg hover:bg-red-700 disabled:bg-gray-700 disabled:cursor-not-allowed transition-colors self-start"
                >
                    {isLoading ? 'Thinking...' : 'Analyze'}
                </button>
            </form>

            <div className="flex-grow bg-black rounded-lg p-4 mt-4">
